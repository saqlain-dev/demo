...
class AirTravelRequestController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = AirTravelRequest::query()->with(['items.department','department','project','accommodation','externalVisitor','airlineCategory'])->get();
...
//            'items.*.estimated_amount' => 'required',
    ...
    public function show($parent_id)
    {
        ...
        $data['parent'] =$parent= AirTravelRequest::query()->with(['items.department','department','project','accommodation','externalVisitor','airlineCategory','airTravelReqVendor.vendorDetail','vendorAtrQuotation.vendorDetail','vendorAtrQuotation.airline','vendorAtrQuotation.airlineCategory','atrInvoice'])->findOrFail($parent_id);
...
//            unset($this->input['items']);
    ...
    public function getDropdowns()
    {
        ...
        $data['vehicles'] = Vehicle::query()->with('VehicleType')->get();
        $data['projects'] = ProjectProfile::approvedProjects();
        $data['drivers'] = Employee::query()->where(['designation_id'=>28, 'employee_type'=>13])->get(['id','name']);
        return resp(1, 'Successful!', $data, Response::HTTP_OK);
    }
