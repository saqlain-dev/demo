...
class ComparativeController extends Controller
{
    ...
    public function getProjectQuotation($id)
    {
        $projectsQuotation= PurchaseRequestRfq::query()
            ...
            }, 'items.itemDetail','vendor_quotations.vendor', 'vendor_quotations.quotationItems.item','purchase_request','disposeRequest'])
            ->find($id);
    ...
    public function shortListVendors(Request $request)
    {
        ...
        if($status == 3){
            ...
            VendorQuotation::query()->where('id',$quotation_id)->update(array('is_qualified'=>1));
        }
    ...
    public function generateComparative(PurchaseRequestRfq $item)
    {
    ...
    public function getResponsiveQuotation($id)
    {
        $data['projectsQuotation']=$projectsQuotation = PurchaseRequestRfq::query()
            ...
            }, 'vendor_quotations.vendor', 'vendor_quotations.quotationItems.item','vendor_quotations.awardQuotation'])
            ->find($id);
    ...
    public function getTenderResponsiveQuotation($id)
    {
        $data['projectsQuotation']=$projectsQuotation = Tender::query()
            ->with(['tenderDetails.itemDetail','purchase_request','vendor_quotations' => function ($query) {
                $query->where('apply_status ', 1);
                $query->where('is_qualified ', 1);

            }, 'vendor_quotations.vendor', 'vendor_quotations.quotationItems.item','vendor_quotations.awardQuotation','tendercommittee.userDetail','tendercommittee.vendorDetail'])
            ->find($id);
