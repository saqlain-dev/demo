...
class VehicleRequestController extends Controller
{
    ...
    public function userVehicleRequest()
    {
        $user_id=auth()->user()->id;
        $employee_id=auth()->user()->employee_id;
        $data = VehicleRequest::with(['created_by','updated_by','VehicleRequestDetail.VehicleId.VehicleType'])->where('created_by',$user_id)->get();
    ...
    public function show(VehicleRequest $vehicleRequest): JsonResponse
    {
        ...
        $data['vehicleRequest']=$vehicleRequest = $vehicleRequest->load(['created_by','updated_by','feedBack.question','feedBack.employee.designation','VehicleRequestDetail.VehicleId.VehicleType','VehicleRequestDetail.procurement','VehicleRequestDetail.procurementDetails.item','vehicleReqQuotations.vendorDetail','vehicleReqQuotations.VehicleRequestDetail','vehicleReqVendor.vendorDetail','vrInvoice','invoices']);
        if($vehicleRequest ){
    ...
    public function sendVehicleRequestForApproval(VehicleRequest $item)
    {
        ...
        $approval_process_name=ApprovalProcessName::query()->where('id',17)->first();
    ...
    function logBook($id) {
        ...
        $data = Vehicle::with(['VehicleType','LogBooks'=>['VehicleType','DriverId','VisitType','created_by','updated_by']])->find($id);
        return resp('1', 'Successful!', $data, Response::HTTP_OK);
    }
    public function logBookFilter(Request $request,$id)
    {
        $this->authorizeAny([
            'log_book_view'
        ]);
        ...
        $data = Vehicle::with([
            'VehicleType',
            'LogBooks' => function ($query) use ($startDate, $endDate) {
                if ($startDate && $endDate) {
                    $query->whereBetween('created_at', [$startDate, $endDate]);
                }
                $query->with(['VehicleType', 'DriverId', 'VisitType', 'created_by', 'updated_by']);
            }
        ])->find($id);
    ...
    public function acceptVRQuotation(Request $request, VendorVehicleReqQuotation $vr)
    {
        ...
        if($vr){
            ...
            try {
                ...
                $vr->quotation_status = $request->quotation_status;
                $vr->save();
                DB::commit();
