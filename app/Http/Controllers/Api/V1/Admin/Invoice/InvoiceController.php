...
class InvoiceController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['invoice_list']=Invoice::query()->with('invoiceItems.itemDetail','grn.grnItem','vendorDetail','consultantContract.CcItems','workOrder.WoItems','rfq.disposeRequest')->get();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $this->input['project_id'] = GRN::where('id', $request->grn_id)
                ->with([
                    'poDetails.rfqDetail.purchase_request.project:id' // Only retrieve project_id
                ])
                ->first()
                ->poDetails
                ->rfqDetail
    ...
    public function show(Invoice $invoice)
    {
        ...
        return resp(1,'Successful!', $invoice->load(['invoiceItems.itemDetail','grn' => ['grnItem','poDetails' => ['PoItems','tenderDetails.purchaseRequest', 'rfqDetail' => ['vendor_quotations', 'purchase_request']]] ,'vendorDetail','consultantContract.CcItems','workOrder' => ['WoItems', 'tenderDetail.purchaseRequest', 'rfqDetail.purchase_request'],'rfq.disposeRequest','ProjectId']),Response::HTTP_OK);    }
        ...
        try {
            ...
            if($allowance_deduction_heads ){
                foreach($allowance_deduction_heads as $key => $allowance_deduction){
                    ...
                    if($account_detail){
                        $account_heads[$key]['id']=$account_detail->id;
                        $account_heads[$key]['head_name']=$account_detail->name;
                        $account_heads[$key]['code']=$account_detail->code;
            ...
            if($salaryaccount_configuration){
                ...
                foreach($salaryaccount_configuration as $key => $configuration){
                    ...
                    if($configuration->id == 4){
                        ...
                        $tax['head_name']=$configuration->ChartOfAccountCode->name;
                        $tax['code']=$configuration->ChartOfAccountCode->code;
                        $tax['id']=$configuration->ChartOfAccountCode->id;
                        ...
                        $eobi['head_name']=$configuration->ChartOfAccountCode->name;
                        $eobi['code']=$configuration->ChartOfAccountCode->code;
                        $eobi['id']=$configuration->ChartOfAccountCode->id;
                        ...
                        $loan['head_name']=$configuration->ChartOfAccountCode->name;
                        $loan['code']=$configuration->ChartOfAccountCode->code;
                        $loan['id']=$configuration->ChartOfAccountCode->id;
                        $loan['amount']=$loan_amount;
                        $loan['type']='Loan';
                        $account_heads[]=$loan;
                        $loan_total=$loan_amount;
                    }else if($configuration->id == 1){
                        $salary['head_name']=$configuration->ChartOfAccountCode->name;
                        $salary['code']=$configuration->ChartOfAccountCode->code;
                        $salary['id']=$configuration->ChartOfAccountCode->id;
                        $salary['amount']=0;
                        $salary['type']=$configuration->account_title;
                        ...
                        $tax['id']=$configuration->ChartOfAccountCode->id;
                        $tax['amount']=0;
                        $tax['type']=$configuration->account_title;
                        $account_heads[]=$tax;
                    }
                ...
                $data['coa']=ChartOfAccount::query()->where('approval_status',1)->get();
