...
class ItemVariantController extends Controller
{
    ...
    {
        $data['list']= ItemVariant::with(['item','project'])->get();
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'project_id' => 'nullable|integer|exists:project_profiles,id',
    ...
    {
        $data['item']= $itemVariant->load(['item','project:id,project_name'])->get();
        ...
        try {
            ...
            for ($i = 0; $i < $quantity; $i++) {
                ...
                if($request->filled('project_id')){
                    $this->input['project_id'] = $request->input('project_id');
                }
                if($request->filled('cost')){
                    $this->input['cost'] = $request->input('cost');
                }
    ...
    {
        $approval_process_name=ApprovalProcessName::query()->where('id',29)->first();
