...
class RoleController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($user && $user->hasRole('Super Admin')) {
            ...
            $this->authorizeAny(['manage_permissions','manage_audit_permissions','manage_roles','manage_audit_roles']);
        }
    ...
    public function show(string $id)
    {
        ...
        if ($user && $user->hasRole('Super Admin')) {
            ...
            // Check if the user has the required permissions
            $this->authorizeAny(['manage_roles','manage_audit_roles']);
        }
    ...
    public function destroy(Role $role)
    {
        $userCount = DB::table('model_has_roles')
            ...
            ->count();
        if ($userCount > 0) {
            return resp(0, 'Role has users and cannot be deleted.', null, Response::HTTP_FORBIDDEN);
        }
        DB::table('role_has_permissions')->where('role_id', $role->id)->delete();
     
        $deleted = $role->delete();
     
        return resp(1, 'Successful!', $deleted, Response::HTTP_OK);
    }
