...
class TenderController extends Controller
{
    ...
    public function getDropdowns()
    {
        ...
        $approvedPr = PurchaseRequest::getApprovedPRs();
        $data['purchase_request']=getRemainingPr($approvedPr);
        return resp(1, 'Successful!', $data, Response::HTTP_OK);
    ...
    public function addMultipleItems(Request $request)
    {
        ...
        $request->validate([
            ...
            'items.*.purchase_request_detail_id' => 'required|integer',
            'items.*.description' => 'required',
            'items.*.unit_of_measurement' => 'required',
            'items.*.required_quantity' => 'required',
        ...
        foreach ($request->items as $item) {
            ...
            $createdItems[] = TenderDetail::create($item);
        }

        $totalAmount = TenderDetail::where('tender_id', $tenderId)->sum('amount');
        Tender::findOrFail($tenderId)->update(['sub_total' => $totalAmount]);

        return resp(1, 'Successful!', $createdItems, Response::HTTP_CREATED);
    }
    ...
    public function sendTenderForApproval(Tender $item)
    {
        ...
        $approval_process_name=ApprovalProcessName::query()->where('id',11)->first();
