...
class VehicleMaintenanceDetailController extends Controller
{
    ...
    public function addMultipleNatureOfWork(Request $request)
    {
        $request->validate([
            'parent_id' => 'required|integer|exists:vehicle_maintenance_forms,id',
            'nature_of_work' => 'required|array',
            'nature_of_work.*.nature_of_work' => 'nullable',
        ]);

        $data = [];
        foreach ($request->nature_of_work as $work) {
            $work['parent_id'] = $request->parent_id;
            $data[] = VehicleMaintenanceDetail::create($work);
        }
