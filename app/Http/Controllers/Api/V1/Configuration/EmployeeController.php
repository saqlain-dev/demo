...
class EmployeeController extends Controller
{
    ...
    public function index()
    {
        ...
        $employeeList= Employee::with(['district','shift','headOffice','branchOffice','designation','marital','salutation','PayscaleLevel','employeeTyp','department','bloodGroupName','parentage','religion','gender','referenceName','user','report','qualification.certification','country','experience','reportTo','grade'])->get();
    ...
    public function addEmployee(){
        ...
        if ($lastEmployee) {
           ...
           // $lastNumber = intval(str_replace('LAS-', '', $lastEmployee->employee_no));
            $lastNumber = $lastEmployee->employee_no;

            // Increment the number by 1
            $newNumber = $lastNumber + 1;

            // Format the new employee number (e.g., LAS-0002)
            $newEmployeeNo =  sprintf('%04d', $newNumber);
        } else {
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
           ...
           // 'district_id' => 'required',
            ...
            'offical_email' => 'required|email|unique:employees,offical_email',
        ...
        if($request->iqama_expiry != ""){
            $this->input['iqama_expiry']=date('Y-m-d',strtotime($request->iqama_expiry));
        }
    ...
    public function show($id)
    {
        ...
        $employee= Employee::with(['district','shift','headOffice','branchOffice','designation','marital','salutation','PayscaleLevel','employeeTyp','department','bloodGroupName','parentage','religion','gender','referenceName','user','report','qualification.certification','experience','reportTo','employeeChnageStatus','grade','employeeDocuments.documentType','empContract.employeeContractDetail.contractType', 'employeeTimesheet.employeeSheetDetail','generatedLetter','country'])->findOrFail($id);

       // $employee= Employee::with(['empContract'])->findOrFail($id);
    ...
    public function update(Request $request, Employee $employee)
    {
        ...
        $request->validate([
           ...
           // 'designation_id' => 'required',
            ...
            'permanent_address' => 'required',
            //'phone_no' => 'required|numeric|digits_between:1,13',
            'offical_email' => 'required|email|unique:employees,offical_email,' .$employee->id. ',id',
        ...
        $employeeDetail=Employee::with(['district','shift','headOffice','branchOffice','designation','marital','employeeTyp','department','bloodGroupName','parentage','religion','gender','referenceName','user','report','qualification','experience','reportTo'])->findOrFail($employee->id);
