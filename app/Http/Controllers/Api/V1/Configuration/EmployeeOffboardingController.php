...
class EmployeeOffboardingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            'employee_id' => 'required|unique:employee_offboardings,employee_id',
    ...
    public function show(EmployeeOffboarding $employee_off_boarding): JsonResponse
    {
        ...
        $offboard_id = $employee_off_boarding->id;
        $data['employee_off_board'] = $employee_off_boarding->load(['interview','Certificate.certificateType','EmployeeId' => ['headOffice','branchOffice','designation','marital','department','gender','report','reportTo','employeeInventory','grade'],'createdBy','updatedBy']);
        $data['issuedBooks'] = BookIssued::where('employee_id', $employee_off_boarding->employee_id)
            ->where('status', 1)->with('book')
            ->get();
        $data['projects'] = $employee_off_boarding->EmployeeId->project_details;
        $data['Reimbursements']=Reimbursement::query()->with(['Expenses'])->withSum('Expenses', 'amount')->where('employee_id',$employee_off_boarding->employee_id)->where('is_voucher_posted',0)->get()->sum('expenses_sum_amount');
        $data['claim_travel_expenses']=ClaimTravelExpense::query()->with(['ExpenseDetail'])->withSum('ExpenseDetail', 'amount')->where('employee_id',$employee_off_boarding->employee_id)->where('is_voucher_posted',0)->get()->sum('expense_detail_sum_amount');
        $data['court_expenses']=CourtExpense::query()->where('employee_id',$employee_off_boarding->employee_id)->where('is_voucher_posted',0);
        $data['advance_salary'] = AdvanceSalary::query()
            ->withSum(['installments' => function ($query) {
                $query->whereNull('PayrollDetailId');
            }], 'due_amount')
            ->where('employee_id', $employee_off_boarding->employee_id)
            ->get();
        $data['issue_items'] = ItemVariant::query()->with('item','inventory','location','assignToEmploy')->where('assign_to_emp',$employee_off_boarding->employee_id)->get();
        ...
        $data['offboard_questions'] = OffboardingQuestion::with(['answers' => function ($q) use ($offboard_id) {
            $q->where('employee_offboard_id', $offboard_id)
    ...
    public function update(Request $request, EmployeeOffboarding $employee_off_boarding)
    {
        ...
        $request->validate([
            'employee_id' => 'required|unique:employee_offboardings,employee_id,'.$employee_off_boarding->id,
            'resignation_date' => 'required|date',
            'reason' => 'required',
        ]);
        if($request->hasFile('attachment')) {
    ...
    public function sendEmployeeOffBoardingForApproval(EmployeeOffboarding $item)
    {
        ...
        $approval_process_name=ApprovalProcessName::query()->where('id',49)->first();
