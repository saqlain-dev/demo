...
class GeneralTemplatesController extends Controller
{
    ...
    public static function getConfigurationDropdown()
    {
        ...
        $data['email_template_type']=Type::getTypeValues('email-template-type');
        $data['general_templates'] = GeneralTemplates::query()->where('status',1)->get();
        $data['employees']= Employee::with(['EmployeeSalary'])->whereNotIn('employee_type', [14, 16, 17, 18])->get();
        $data['approval_process_lists']= ApprovalProcessName::query()->get();
