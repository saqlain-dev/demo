...
class GeneratedLetterController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['items'] = GeneratedLetter::with(['TemplateId', 'EmployeeId', 'created_by', 'updated_by'])->select('id', 'template_id', 'employee_id', 'letter_name', 'status', 'letter_reference', 'letter_no', 'created_at', 'updated_at', 'created_by', 'updated_by', 'attachment', 'is_system_generated','date','acknowledgment_status','remarks')->where('is_system_generated', 1)->get();
    ...
    public function getTemplateContent1(Request $request)
    {
        ...
        if ($request->filled('vendor_id')) {
            ...
            $vendorFields = [
                ...
                'telephone_1',
                'fax_no',
                'main_area_of_business',
                'other_area_of_business',
                'year_in_business',
                'ntn_number',
                'email'
            ];
        ...
        if ($request->filled('employee_id')) {
            ...
            $employeeFields = [
                ...
                'cnic' => 'Cnic',
                'phone_no' => 'PhoneNo',
                'cnic_issuance' => 'CnicIssuance',
                'cnic_expiry' => 'CnicExpiry',
                'personal_email' => 'PersonalEmail',
                'offical_email' => 'OfficalEmail',
                'department_id' => 'Department',
                'report_to_id' => 'ReportTo',
    ...
    public function updateLetter(Request $request, GeneratedLetter $generatedLetter)
    {
        $this->authorize('generated_letter_update');

        $validated = $request->validate([
            'acknowledgment_status' => 'nullable',
            'date' => 'nullable|date',
        ...
        try {
            ...
            $generatedLetter->update([
                'acknowledgment_status' => $validated['acknowledgment_status'] ?? null,
    ...
    public function saveSystemGeneratedLetter(Request $request): JsonResponse
    {
        ...
        try {
            ...
            if ($request->file('attachment') && $request->file('attachment')->isValid()) {
                $responce = $this->saveFile($request, 'GeneratedLetters');
                $data['item']->update(['attachment' => $responce]);
            }
