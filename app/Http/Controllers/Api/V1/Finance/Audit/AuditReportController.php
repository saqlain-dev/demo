...
use App\Models\Finance\Audit\AuditReport;
...
class AuditReportController extends Controller
{
    ...
    {
        $data = AuditReport::with(['DraftBy','created_by','updated_by'])->get();
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('attachment')) {
            $responses = $this->saveImage($request, 'audit_report');
            $this->input['attachment'] = $responses;
        }
        try {
            DB::beginTransaction();
            $item = AuditReport::query()->create($this->input);
            DB::commit();
    ...
    {
        $grantAppreciationLetter = $auditReport->load(['DraftBy','created_by','updated_by']);
    ...
    public function update(Request $request, AuditReport $auditReport)
    {
        ...
        try {
            DB::beginTransaction();
            $item = $auditReport->update($this->input);
            DB::commit();
