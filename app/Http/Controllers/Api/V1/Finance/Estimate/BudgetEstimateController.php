...
class BudgetEstimateController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['invoice_list'] = Invoice::query()
            ->with('invoiceItems.itemDetail', 'grn.grnItem', 'vendorDetail', 'consultantContract.CcItems', 'workOrder.WoItems','invoiceFuelRequest')
            ->where('invoice_status',1)
            ->where('is_voucher_posted', 0)
            ->whereNotIn('id', function($query) {
                $query->select('VoucherFromID')
                    ->from('voucher_details') // 👈 Updated to the child table
        ...
        $data['pending-court-expense'] = CourtAdvocateExpense::query()
            ->with(['employee.designation', 'purchaseRequest', 'created_by', 'updated_by'])
            ->where(['approval_status' => 1])
            ->whereNotIn('id', function($query) {
                $query->select('VoucherFromID')
                    ->from('voucher_details')
        ...
        $data['advanced_salary'] = AdvanceSalary::query()
            ...
            ->get();
            $data['las_invoices'] = LasInvoice::query()
                ->with(['InvoiceDetail','BankId','customerable'])
                ->where('approval_status', 1)
                ->whereNotIn('id', function($query) {
                    $query->select('VoucherFromID')
                        ->from('voucher_details')
                ...
                ->withSum('InvoiceDetail', 'amount')
                ->get();

        $data['air_travel_request'] = AirTravelRequest::query()
            ->with([
                'accommodation',
                'department',
                'airlineCategory',
            ...
            ->whereNotIn('id', function ($query) {
                $query->select('VoucherFromID')
                    ->from('voucher_details')
                    ->where('VoucherFrom', 'ATR');
            })
            ->whereHas('vendorAtrQuotation', function ($query) {
                $query->where('quotation_status', 1);
            })
            ->get();

        $data['fuel_request'] = FuelRequest::query()
            ->with([
                'FuelConsumption',
                'ProjectId',
                'VehicleId',
            ...
            ->whereNotIn('id', function ($query) {
                $query->select('VoucherFromID')
                    ->from('voucher_details')
                    ->where('VoucherFrom', 'FUEL');
            })
            ->get();


        /*$data['vehicle_maintenance'] = VehicleMaintenanceForm::query()
            ->with(['items','vehicle','department','created_by','vehicleMaintenanceVendor','vehicleMaintenanceQuotations.vendorDetail',
                'vehicleMaintenanceQuotations'=> function ($query) {
                $query->where('quotation_status', 1);
            }
            ])
            ->where('approval_status', 1)
            ->whereNotIn('id', function($query) {
                $query->select('VoucherFromID')
                    ->from('voucher_details')
                    ->where('VoucherFrom', 'VM');
            })
            ->withSum('items', 'estimated_expenditure')
            ->get();*/
        $data['vehicle_maintenance'] = VehicleMaintenanceForm::query()
            ->with([
                'items', 'vehicle', 'department', 'created_by',
                'vehicleMaintenanceVendor',
            ...
            ->whereHas('vehicleMaintenanceQuotations', function ($query) {
                $query->where('quotation_status', 1);
            })
            ->withSum('items', 'estimated_expenditure')
            ->get();
