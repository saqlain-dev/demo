...
class LasInvoiceController extends Controller
{
    ...
    {
        $data['listing'] = LasInvoice::query()->with(['BankId','customerable','created_by','updated_by','InvoiceDetail'])->get();
...
//            }
    ...
    {
        $data['las_invoice'] = $lasInvoice = $lasInvoice->load(['BankId','customerable','created_by','updated_by','InvoiceDetail.UnitType']);
        //dd($data['las_invoice']->customerable); // Debug check here
...
//            }
            $lasInvoice->refresh();
            DB::commit();
            return resp(1, 'Successful!', $lasInvoice->load('customerable','coaDetail','coaClassDetail'), Response::HTTP_CREATED);
        } catch (\Exception $e) {
            DB::rollBack();
