...
class MinuteOfMeetingController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['minuteOfMeeting_list'] = MinuteOfMeeting::query()->select('mom_document','board_meeting_id','id')->with([
    ...
    public function meetListFilterd(Request $request)
    {
        $request->validate([
        ...
        $data['minuteOfMeeting_list'] = MinuteOfMeeting::query()
            ->select('mom_document', 'board_meeting_id', 'id')
            ->whereHas('boardMeetingDetail', function ($query) use ($request) {
                $query->whereBetween('board_meeting_date', [$request->start_date, $request->end_date]);
            })
            ->with([
                'boardMeetingDetail.agendaDetail',
                'boardMeetingApplicants' => function ($query) {
    ...
    public function momDropDown()
    {
        ...
        $data['board_meetings_list']=BoardMeeting::query()->with(['agendaDetail'])->get();
        return resp(1, 'Successful!', $data, Response::HTTP_OK);
    }
    public function minutesOfMeetingDropDown()
    {
        $this->authorize('board_meeting_view');

        $data['board_meetings_list']=BoardMeeting::query()->with(['agendaDetail','BoardMeetingMom'])->where('approval_status',1)->whereDoesntHave('BoardMeetingMom')->get();
        return resp(1, 'Successful!', $data, Response::HTTP_OK);
    }
