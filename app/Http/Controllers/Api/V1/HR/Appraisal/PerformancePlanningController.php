...
class PerformancePlanningController extends Controller
{
    ...
    public function show(PerformancePlanning $PerformancePlanning)
    {
        ...
        $item = $PerformancePlanning->load(['employeeWorkplan.employeeWorkplanActivity', 'employee' => ['designation', 'department', 'shift'], 'supervisorImmediate', 'supervisorExtended', 'keyResponsibilities' => ['questionSection.Kpis.DesignationId'], 'comments.createdBy', 'developmentGoals', 'performanceFactors' => ['questionSection.Kpis.DesignationId']]);
    ...
    {
        $approval_process_name=ApprovalProcessName::query()->where('id',32)->first();
