...
class ComplaintController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['complaint_list']=Complaint::query()->whereIn('complaint_status',[0, 1, 2, 3])->with('employee','complainFrom','complainAgainst','department','natureOfComplaint','complaintMeeting')->get();
    ...
    public function show(Complaint $complaint)
    {
        ...
        $data['view_complaint']=$complaint->load('comments.createdBy','employee','complainFrom','department','natureOfComplaint','committeeMembers.memeberDetail','committeeMembers.complaintLetter','complaintMeeting');
        ...
        if ($complaint->complain_type == 0){ // against employee
            $history = Employee::query()->with(['complainAgainsts' => ['complainFrom','department','complainAgainst']])->find($complaint->complain_from_emp);
            $data['view_complaint']->complain_againsts = $history->complainAgainsts;
        }
        ...
        $data['view_complaint']->complain_from_employees = $history->complainFromEmployees;
    ...
    public function employeeAssignedComplaints(Employee $employee)
    {

        $complaints = Complaint::where('employee_id', $employee->id)
            ->with('complainFrom', 'complainAgainst', 'department', 'natureOfComplaint')
            ->latest()
            ->get();

        return resp('1', 'Employee complaints fetched successfully.', $complaints);
    }

    public function updateEmployeeComplaint(Complaint $complaint,Request $request)
    {
        $this->authorizeAny([
            'complaint_update',
            'manage_employee_portal',
        ...
        try {
            ...
            $validated = $request->validate([
                ...
                'employee_status' => 'nullable|integer|in:1,2,3,4,5,6',
                'employee_remarks' => 'nullable|string',
            ]);
            // Validate at least one of the three fields is present  
            if (!$request->filled('employee_id')&!$request->filled('employee_status')&!$request->filled('employee_remarks')) {
                return resp('0', 'At least one of employee_id, employee_status, or employee_remarks is required.', null, Response::HTTP_UNPROCESSABLE_ENTITY);
            }

            // Prevent updating status/remarks before assignment
            if ($request->hasAny(['employee_status', 'employee_remarks']) && !$complaint->employee_id && !$request->employee_id) {
                return resp('0', 'Assign an employee before updating status or remarks.', null, Response::HTTP_UNPROCESSABLE_ENTITY);
            }
            ...
            DB::rollBack();
            return resp('0', 'Failed to update complaint. Error: ' . $e->getMessage(), null, Response::HTTP_INTERNAL_SERVER_ERROR);
        }
    }
    ...
    {
        $approval_process_name=ApprovalProcessName::query()->where('id',20)->first();
    ...
    public function createCommittee(Request $request,Complaint $item)
    {
        ...
        try {
            ...
            $request->validate([
                ...
                'complaint_meeting_id' => 'required|integer',
            ]);
            ...
            foreach($committeeMembers as $member){
                $insert=array(
                    ...
                    'complaint_meeting_id'=>$request->complaint_meeting_id,
                );
