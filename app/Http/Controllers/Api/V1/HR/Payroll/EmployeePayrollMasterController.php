...
class EmployeePayrollMasterController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['payroll_listing']=EmployeePayrollMaster::query()->with('createdBy')->orderByDesc('id')->get();
    ...
    public function generatePayroll(Request $request)
    {
        ...
        $department = !empty($this->input['department']) ? $this->input['department'] : null;
        $employeetype = !empty($this->input['employee_type']) ? $this->input['employee_type'] : null;
        $branch_office = !empty($this->input['branch_office']) ? $this->input['branch_office'] : null;
        $gender = !empty($this->input['gender']) ? $this->input['gender'] : null;
...
/*              $tax_Deduction['AllowedLiters'] = NULL;
    ...
    {
        $approval_process_name=ApprovalProcessName::query()->where('id',31)->first();
        ...
        if($approval_process->count() > 0 && $checkProcess == 0){
            ...
            foreach ($approval_process as $approval){
                ...
                if($approval_process_name->isFinancialApproval == 1){
                    if($approval->financialAmount < $item->TotalNetPay  ){
    ...
    public function getEmployeePayroll(Request $request)
    {
        ...
        try {
            // $data['employee_salary']=EmployeePayrollDetail::query()->with(['salaryAllownaceDeduction','salarySegregation.projectDetail','BankDetail','preGrossSalaryAllowances','employeeDetail' => ['designation','grade','department','branchOffice','district']])->where(['EmployeeId'=>$request->employee_id, 'PayperiodTo' => $request->pay_month])->first();
            $data['employee_salary'] = EmployeePayrollDetail::query()
                ->with([
                    'salaryAllownaceDeduction',
                    'salarySegregation.projectDetail',
                    'BankDetail',
                    'preGrossSalaryAllowances',
                    'employeeDetail' => ['designation', 'grade', 'department', 'branchOffice', 'district']
                ])
                ->where('EmployeeId', $request->employee_id)
                ->whereMonth('PayperiodTo', '=', date('m', strtotime($request->pay_month)))
                ->whereYear('PayperiodTo', '=', date('Y', strtotime($request->pay_month)))
                ->first();
    ...
    public function updatePayrollAuditDetail(Request $request)
    {
        $request->validate([
        ...
        try {
            ...
            EmployeePayrollDetail::query()->where('id', $request->payroll_detail_id)->update($update);
            $item=EmployeePayrollDetail::query()->where('id', $request->payroll_detail_id)->first();
            DB::commit();
