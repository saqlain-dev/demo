...
class EmployeeSalarySetupController extends Controller
{
    ...
    public function updateSalarySetup(Request $request)
    {

        /*$request->validate([
            'salary_setup_id' => 'required',
            'coa_id' => 'required',
        ]);*/
        $request->validate([
            '*.salary_setup_id' => 'required|integer', // Validate salary_setup_id for each object
            '*.coa_id' => 'required|integer', // Validate coa_id for each object
        ], [
            '*.salary_setup_id.required' => 'The salary setup ID is required for each item.',
            '*.coa_id.required' => 'The COA ID is required for each item.',
        ]);
        ...
        try {
            ...
            foreach ($data as $item) {
                EmployeeSalarySetup::query()
                    ->where('id', $item['salary_setup_id']) // Match the record by salary_setup_id
                    ->update(['coa_id' => $item['coa_id']]); // Update coa_id
            }
            $data['employee_list']=EmployeeSalarySetup::query()->with('employeeDetail','salarySegregation.projectDetail','bankDetail')->get();
            DB::commit();
            return resp('1', 'Employee salary updated Successfully!', $data, Response::HTTP_CREATED);
    ...
    public function salaryCoaSetupDropDown()
    {
        $data['employee_salaries'] = EmployeeSalarySetup::query()->with('employeeDetail.designation','coa')
            ...
            ->get();
        $data['coa']=ChartOfAccount::query()->with('AccountTypeId')->where('approval_status',1)->get();
        return resp(1, 'Successful!', $data, Response::HTTP_OK);
    }
