...
class EmployeeRequisitionController extends Controller
{
    ...
    public function show(EmployeeRequisition $employeeRequisition): JsonResponse
    {
        ...
        $requisition_process=($employeeRequisition->is_consultant == 1)? 59 : 30;
        ...
        $data['approval_request']=getNextApproval($requisition_process,auth()->user()->designation_id,$employeeRequisition->id);
        $data['approval_request_status']=checkApprovalRequestStatus($requisition_process,$employeeRequisition->id);
        return resp('1', 'Successful!', $data, Response::HTTP_OK);
    }
...
//            'required_skills' => 'required',
    ...
    {
        $requisition_process=($item->is_consultant == 1)? 59 : 30;
        $approval_process_name=ApprovalProcessName::query()->where('id',$requisition_process)->first();
        $approval_process=ApprovalProcess::query()->where('approval_process_id',$requisition_process)->get();
        $checkProcess=ApprovalProcessList::query()->where('approval_process_id',$requisition_process)->where('approval_request_status',1)->where('request_module_id',$item->id)->count();
