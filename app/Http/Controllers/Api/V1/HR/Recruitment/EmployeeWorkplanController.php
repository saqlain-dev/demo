...
//            $item->sectionQuestions()->sync($section_questions);
    ...
    public function show(EmployeeWorkplan $employeeWorkplan): JsonResponse
    {
        ...
        $employeeWorkplan = $employeeWorkplan->load(['employeeWorkplanActivity' => ['KpiId', 'sectionQuestions'],'EmployeeId','ProjectId','created_by','updated_by','sectionQuestions']);
    ...
    public function getEmployeeWorkplan($empId): JsonResponse
    {
        ...
        $data['items'] = EmployeeWorkplan::query()
            ...
            ->whereHas('EmployeeId', function ($query)use ($empId) {
                $query->where('id', $empId); // Replace 'your_column' and 'your_value' with your actual column and value
            })
            ->get();
        return resp('1', 'Successful!', $data, Response::HTTP_OK);
    }
