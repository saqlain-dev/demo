...
class ManageJobController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = ManageJob::with(['DepartmentId','RequiredJobType','RequisitionId','created_by','updated_by','applierType'])->get();
    ...
    public function viewJobs()
    {
        ...
        $data = ManageJob::with(['DepartmentId','RequiredJobType','created_by','updated_by','applierType'])->get();
        return resp('1', 'Successful!', $data, Response::HTTP_OK);
    }
    public function saveApplyJobFiles($request,$folder){

        $file = $request->file('technical_proposal');
    ...
    public function saveApplyJobFinancialFiles($request,$folder){

        $file = $request->file('financial_proposal');
        $path = 'uploads/media/' . $folder;
        if (!file_exists('uploads')) {
            mkdir('uploads', 0777, true);
        }
    ...
    public function show(ManageJob $manageJob): JsonResponse
    {
        ...
        $manageJob = $manageJob->load(['DepartmentId','RequiredJobType','RequisitionId','created_by','updated_by','applierType']);
    ...
    public function applyOnJob(Request $request)
    {
        ...
        if($request->hasFile('technical_proposal')) {

            $responce = $this->saveApplyJobFiles($request, 'technicalProposal');

            if ($responce) {
                $this->input['technical_proposal'] = $responce;
            }
        }else{
            unset($this->input['technical_proposal']);
        }
        if($request->hasFile('financial_proposal')) {

            $responce = $this->saveApplyJobFinancialFiles($request, 'financialProposal');

            if ($responce) {
                $this->input['financial_proposal'] = $responce;
            }
        }else{
            unset($this->input['financial_proposal']);
        }
        try {
            DB::beginTransaction();
    ...
    public function hrDropdown(){
        ...
        $data['applier_type']= Type::getTypeValues('applier-type');
        ...
        $data['job_requisitions']= EmployeeRequisition::query()->where('status',1)->doesntHave('Jobs')->get();
        $data['job_mode']= Type::getTypeValues('job-mode');
        $data['employees']= Employee::with(['EmployeeSalary','employeeTyp'])->whereNotIn('employee_type', [14, 16, 17, 18])->get();
    ...
    public function getHrPendingApproval()
    {
