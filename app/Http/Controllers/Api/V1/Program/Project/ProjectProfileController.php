...
class ProjectProfileController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['project_profiles'] = ProjectProfile::with([
            ...
            'thematic_area','status','project_manager','pdu_focal_person','implementing_partner','created_by','updated_by','donor','provinces','comments.createdBy'
        ])->orderByDesc('id')->get();
    ...
    public function getProjectProfilesRRFReport()
    {
        ...
        $data['project_profiles'] = ProjectProfile::with([
            ...
            'thematic_area','status','project_manager','pdu_focal_person','implementing_partner','provinces','created_by','updated_by','donor.donorDetail','comments.createdBy'
...
//        $data['projectRrfRejectedCount']= $data['project_profiles']->where('approval_status',1)->where('project_rrf_approval',3)->count();
    ...
    public function store(Request $request)
    {
        ...
        $item->provinces()->sync($request->province_ids);
    ...
    public function show(ProjectProfile $projectProfile)
    {
        ...
        $data['projectProfile'] = $projectProfile->load([
            ...
            'thematic_area','status','project_manager','pdu_focal_person','implementing_partner','created_by','updated_by','donor.donorDetail','provinces','currency',
        ]);
    ...
    public function update(Request $request, ProjectProfile $projectProfile)
    {
        ...
        $projectProfile->provinces()->sync($request->province_ids);
    ...
    public function sendProjectForApproval(ProjectProfile $item)
    {
        ...
        $approval_process_name=ApprovalProcessName::query()->where('id',4)->first();
        ...
        if($approval_process->count() > 0 && $checkProcess == 0){
            ...
            foreach ($approval_process as $approval){
                ...
                if($approval_process_name->isFinancialApproval == 1){
                    if($approval->financialAmount < $item->budget  ){
    ...
    {
        $this->authorize('project_view');
        $approval_process_name=ApprovalProcessName::query()->where('id',6)->first();
