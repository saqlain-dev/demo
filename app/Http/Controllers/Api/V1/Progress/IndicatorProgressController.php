...
class IndicatorProgressController extends Controller
{
    ...
    public function getIndicatorProgressWkpid($wkpid)
    {
        ...
        try {
            $indicatorProgress = IndicatorProgress::with(['IndicatorMovs', 'ProgressWorkplanId', 'ProgressStatus'])->where('progress_workplan_id', $wkpid)->get();
            if ($indicatorProgress->isEmpty()) {
                return resp('1', 'No indicator progress found.', [], Response::HTTP_OK);
            }

            $ind = IndicatorProgressResource::collection($indicatorProgress);

            return resp('1', 'Successful!', $ind, Response::HTTP_OK);
        } catch (\Throwable $th) {
            //throw $th;
            // Log::error($th);
            return resp('0', $th->getMessage(), [], Response::HTTP_BAD_REQUEST);
        }
    ...
    public function update(Request $request, IndicatorProgress $indicatorProgress)
    {
        ...
        $indicatorProgress->update($validated);
        return resp('1', 'Record Updated Successfully!', $indicatorProgress, Response::HTTP_CREATED);
    }
    /**
    ...
    public function getFormResponses($indicatorProgressId, $formId)
    {
        ...
        $indicatorProgress = new IndicatorProgressResource(IndicatorProgress::with([
            'IndicatorMovs',
            'ProgressWorkplanId',
            'ProgressStatus',
            'questionnaires' => function ($query) use ($formId) {
                $query->where('form_id', $formId);
                $query->with('answers.question');
            },
            'ProgressWorkplanId',
            'ProgressStatus'
        ])->findOrFail($indicatorProgressId));
