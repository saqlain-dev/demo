...
class ProgressWorkplanController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['work_plan_list'] =$work_plan_list= ProgressWorkplan::with(['workPlanGoals'=>['GoalId','GoalIndicatorId','activities'],'workPlanOutcome'=>['OutcomeId','OutcomeIndicatorId','activities'],'workPlanOutput'=>['OutputId','OutputIndicatorId','activities'],'CreatedBy','UpdatedBy','comments.createdBy'])->orderByDesc('id')->get();
...
//        }
    ...
    public function getWorkplanbyWkplid($wkpid)
    {
        ...
        $data['projectDetail'] = ProjectProfile::with(['donor.donorDetail',
            'projectGoals' =>
                [   'lasSpDetail',
                    'ProGoalIndicators' =>['proWorkPlanIndicators.GoalStatus','proGoalIndicatorTargets'],
                    'projectOutcomes' => [
...
//        }
    ...
    public function getWorkPlanOutcomesbyWkpid(Request $request, $wkpid)
    {
        ...
        $data['projectOutcomeDetail'] = ProjectRrfOutcome::with([
                        'ProOutcomeIndicators' => ['proWorkPlanIndicators.OutcomeStatus','proOutcomeIndicatorTargets'],
                        'lasSpDetail',
                        'project_rrf_goal'
...
//        }
    ...
    public function getWorkPlanOutputByWkpid(Request $request, $wkpid)
    {
        ...
        $data['workplan'] = ProgressWorkplan::find($wkpid);
        $projectId = $data['workplan']->project_id;
        $data['projectOutputDetail'] = ProjectRrfOutput::with([ 'ProOutputIndicators' => ['proOutputIndicatorTargets','proWorkPlanIndicators' =>['OutputStatus','activities']],'lasSpDetail','project_rrf_outcome'
        ])->where('project_id',$projectId)->get();
        return resp('1', 'Successful!', $data, Response::HTTP_OK);
    }
    ...
    public function updateWorkPlanOutput(Request $request, ProgressWorkplanOutput $ProgressWorkplanOutput)
    {
        ...
        $this->input['output_timeline_for_indicators'] = json_encode($this->input['output_timeline_for_indicators']);
        $this->input['target'] = json_encode($this->input['target']);
        $outputIndicatorId= $this->input['output_indicator_id'];
        ...
        try {
            ...
            $this->updateOutputIndicatorProgress($this->input);

            //$this->updateOutcomeIndicatorProgress($outputIndicatorId);
            $this->updateLasRrfOutputIndicatorProgress($outputIndicatorId);
            DB::commit();
    ...
    public function updateLasRrfOutputIndicatorProgress($outputIndicatorId)
    {
        $outputIndicator = ProjectRrfOutputIndicator::query()->where('id', $outputIndicatorId)->first();
        if (!$outputIndicator) {
            return;
        }
    ...
    public function updateOutputIndicatorProgress($inputData)
    {
        $target = $inputData['target'] ?? [];
        if (is_string($target)) {
            $target = json_decode($target, true); // convert JSON string → array
        }
        foreach ($target as $year => $quarters) {
            $yearlyTarget = 0;
            $yearlyProgress = 0;

            foreach ($quarters as $quarter) {
                $yearlyTarget   += (float) ($quarter['value'] ?? 0);
                $yearlyProgress += (float) ($quarter['progress'] ?? 0);
            }

            DB::table('project_rrf_output_indicator_targets')
                ->where('project_rrf_output_indicator_id', $inputData['output_indicator_id'])
                ->where('year', $year)
                ->update([
    ...
    public function updateOutcomeIndicatorProgress($outputIndicatorId)
    {
        // 1. Get the output indicator and its parent outcome
        $outputIndicator = ProjectRrfOutputIndicator::query()->where('id', $outputIndicatorId)->first();
        if (!$outputIndicator) {
            return;
        }

        $outcomeIndicatorId = $outputIndicator->project_rrf_outcome_indicator_id;

        // 3. Now aggregate all outputs under this outcome
        $outputYearly = DB::table('project_rrf_output_indicator_targets as t')
            ->join('project_rrf_output_indicators as oi', 't.project_rrf_output_indicator_id', '=', 'oi.id')
            ->where('oi.project_rrf_outcome_indicator_id', $outcomeIndicatorId)
            ->select('t.year', DB::raw('SUM(t.progress) as total_progress'))
            ->groupBy('t.year')
            ->get();

        // 4. Update outcome indicator yearly targets
        foreach ($outputYearly as $row) {
            DB::table('project_rrf_outcome_indicator_targets')
                ->where('project_rrf_outcome_indicator_id', $outcomeIndicatorId)
                ->where('year', $row->year)
                ->update([
                    'progress'   => $row->total_progress,
                    'updated_at' => now()
                ]);
        }
        $this->updateGoalIndicatorProgress($outcomeIndicatorId);
    }

    public function updateGoalIndicatorProgress($outcomeIndicatorId)
    {
        // 1. Get the outcome indicator and its parent goal
        $outcomeIndicator = ProjectRrfOutcomeIndicator::query()->where('id', $outcomeIndicatorId)->first();
        if (!$outcomeIndicator) {
            return;
        }

        $goalIndicatorId = $outcomeIndicator->project_rrf_goal_indicator_id;

        // 2. Aggregate all outcome yearly progress under this goal
        $outcomeYearly = DB::table('project_rrf_outcome_indicator_targets as t')
            ->join('project_rrf_outcome_indicators as oi', 't.project_rrf_outcome_indicator_id', '=', 'oi.id')
            ->where('oi.project_rrf_goal_indicator_id', $goalIndicatorId)
            ->select('t.year', DB::raw('SUM(t.progress) as total_progress'))
            ->groupBy('t.year')
            ->get();

        // 3. Update goal indicator yearly targets
        foreach ($outcomeYearly as $row) {
            DB::table('project_rrf_goal_indicator_targets')
                ->where('project_rrf_goal_indicator_id', $goalIndicatorId)
                ->where('year', $row->year)
                ->update([
                    'progress'   => $row->total_progress,
                    'updated_at' => now()
    ...
    public function getDropDowns(): JsonResponse
    {
        ...
        $data['projects'] = ProjectProfile::leftJoin('progress_workplans', function ($join) {
            $join->on('project_profiles.id', '=', 'progress_workplans.project_id')
                ->whereNull('progress_workplans.deleted_at'); // exclude soft-deleted workplans
        })
        ->whereNull('progress_workplans.project_id')
        ->where('project_profiles.project_rrf_approval', 1)
    ...
    {
        $this->authorize('work_plan');
        $approval_process_name=ApprovalProcessName::query()->where('id',7)->first();
    ...
    public function getProjectProgress(Request $request)
    {
        ...
        try {
            DB::beginTransaction();
            $rawData= DB::select('EXEC GetProjectYearlyProgress ?', [$request->project_id]);
            $data['project_progress'] = json_decode(json_encode($rawData), true);
            DB::commit();
    ...
    public function getProjectGoalProgress(Request $request)
    {
        $request->validate([
            'goal_id' => 'required'
        ]);
        ...
        try {
            ...
            $goalDetail=ProjectRrfGoal::query()->with()->where('id',$request->goal_id)->first();
            $rawData= DB::select('EXEC GetGoalYearlyProgress ?', [$request->goal_id]);
            $data['project_goal_progress'] = json_decode(json_encode($rawData), true);
            DB::commit();
            return resp('1', 'Successfully!', $data, Response::HTTP_OK);
        } catch (\Exception $e) {
            DB::rollBack();
