...
//               }
    ...
    public function show(string $id)
    {
        ...
        $data['responce']= StrategicPlan::with(['pillars.indicators' => ['indicatorYears']])->find($id);
...
//                    }
       ...
       if ($item->approval_process_id == 42) {
        ...
        if ($item->approval_process_id == 53) {
            ...
            if ($request->approval_status == 3) {
                ...
                CourtExpense::query()->where('id', $item->request_module_id)->update(array('approval_status' => 3));
                ...
                if (isNull($CheckNextApprovalRecord) && empty($CheckNextApprovalRecord)) {
                    ...
                    CourtExpense::query()->where('id', $item->request_module_id)->update(array('approval_status' => 1));
        ...
        if ($item->approval_process_id == 61) {
            ...
            if ($request->approval_status == 3) {
                ...
                LeadQualification::query()->where('id', $item->request_module_id)->update(array('approval_status' => 3));

                $this->rejectLead($item->request_module_id);

                $createdBy = LeadQualification::where('id', $item->request_module_id)->pluck('created_by');

                notifyRequester($createdBy,$item->approval_process_id);
            } else {
                ...
                if (isNull($CheckNextApprovalRecord) && empty($CheckNextApprovalRecord)) {
                    ...
                    LeadQualification::query()->where('id', $item->request_module_id)->update(array('approval_status' => 1));
                    $this->convertLeadToOpportunity($item->request_module_id);
                    $this->rejectLead($item->request_module_id);
        ...
        if ($item->approval_process_id == 62) {
            ...
            if ($request->approval_status == 3) {
                ...
                Quotation::query()->where('id', $item->request_module_id)->update(array('approval_status' => 3));
                ...
                if (isNull($CheckNextApprovalRecord) && empty($CheckNextApprovalRecord)) {
                    ...
                    Quotation::query()->where('id', $item->request_module_id)->update(array('approval_status' => 1));

                    $createdBy = Quotation::where('id', $item->request_module_id)->pluck('created_by');
    ...
    public function convertLeadToOpportunity($qualification_id)
    {
        $leadQualification=LeadQualification::query()->find($qualification_id);
        if($leadQualification){
            $lead=Lead::query()->where('id',$leadQualification->lead_id)->first();
            $lead->lead_status=692;
            $lead->save();
            $lead->refresh();
            if($lead){
                $opp_activity_count=0;
                if(!empty($lead->activity_id)){
                    $opp_activity_count=1;
                }
                ...
                $insert=array(
                'opportunityable_type'=>Lead::class,
                'opportunityable_id'=>$lead->id,
                'opportunity_name'=>$lead->job_title,
                'source'=>$lead->source,
                'opportunity_owner'=>$leadQualification->assign_to,
                'opportunity_amount'=>$lead->estimate_budget,
                'closing_date'=>$lead->lead_date,
                'stage_rating'=>2,
                'opportunity_type'=>675,
                'opp_activity_count'=>$opp_activity_count,
                'series'=>$opp_series_number,
                );
                Opportunity::query()->create($insert);
            }
        }

    }

    public function rejectLead($qualification_id)
    {
        $leadQualification=LeadQualification::query()->find($qualification_id);
        if($leadQualification){
            $lead=Lead::query()->where('id',$leadQualification->lead_id)->first();
            $lead->lead_status=691;
            $lead->save();
            $lead->refresh();
        }
