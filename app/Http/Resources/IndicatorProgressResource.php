...
class IndicatorProgressResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        if ($this->type_of_indicator == 1) {
            $indicator_type = 'Goal';
            $indicator_parent = ProgressWorkplanGoals::query()->with('GoalStatus')->find($this->type_id);
            if ($indicator_parent && $indicator_parent->goal_movs_ids) {
            ...
            $indicator_parent = ProgressWorkplanOutcome::query()->with('OutcomeStatus')->find($this->type_id);

            if($indicator_parent && $indicator_parent->outcome_movs_ids) {
            ...
            $indicator_parent = ProgressWorkplanOutput::query()->with('OutputStatus')->find($this->type_id);
            if ($indicator_parent && $indicator_parent->output_movs_ids) {
        ...
        $workplanTools = is_array($this->form_id)
                            ? QuestionnaireForm::whereIn('id', $this->form_id)->get()
                            : [];

        $reportingLevel = is_array($this->reporting_level)
                            ? TypeValue::whereIn('id', $this->reporting_level)->get()
                            : [];
        return [
            'id' => $this->id,
            'progress_workplan_id' => $this->ProgressWorkplanId,
            'type_of_indicator' => $indicator_type ?? null,
            ...
            'IndicatorMovs' => $this->IndicatorMovs ?? [],
